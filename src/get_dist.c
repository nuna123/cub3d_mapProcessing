...
static void	horiz_init(t_gameInfo *gi, double a, double pl[2], double dot[2])
{
	...
	dot[0] = (((int)(pl[0] / gi->texture_size))
			+ (a < 90 || a > 270)) * gi->texture_size;
...
double	get_horiz_dist(t_gameInfo *gi, double a)
{
	...
	dis_diff = fabs(gi->texture_size / cos(dtr(a)));
	dis = fabs(((pl[0] - dot[0]) / cos(dtr(a))));
	diff[1] = -1 * sin(dtr(a)) * dis_diff;
	diff[0] = gi->texture_size - (((a > 90 && a < 270)) * gi->texture_size * 2);
	while (dot[0] < gi->screen_w && dot[0] > 0 && dot[1] > 0 && dot[1] < gi->screen_h)
	{

		if (coors_in_map(gi, dot[0] - (a > 90 && a < 270),
				dot[1] - ( (fmod(dot[1], gi->texture_size) == 0) && (a < 180))) != '0')
			break;
		dot[0] += diff[0];
		dot[1] += diff[1];
		dis += dis_diff;
...
/*
	...
	diff[1] is how much to add to the dot[1] pos in each loop == gi->texture_size
...
static void	vert_init(t_gameInfo *gi, double a, double pl[2], double dot[2])
{
	...
	dot[1] = (((int)(pl[1] / gi->texture_size) + (a > 180))) * gi->texture_size;
...
double	get_vert_dist(t_gameInfo *gi, double a)
{
	...
	dis_diff = fabs((gi->texture_size / sin(dtr(a))));
	diff[0] = dis_diff * cos (dtr(a));
	dis = fabs((fabs(pl[1] - dot[1]) / sin(dtr(180 - a))));
	diff[1] = gi->texture_size - (a < 180) * gi->texture_size * 2;
	while (dot[0] < gi->screen_w && dot[0] > 0 && dot[1] > 0 && dot[1] < gi->screen_h)
	{
		if (coors_in_map (gi, dot[0] - ((fmod(dot[0], gi->texture_size) == 0) && (a > 90 && a < 270)),
				dot[1] - (a < 180)) != '0')
			break;
		dot[0] += diff[0];
		dot[1] += diff[1];
